...
export const updateUserActionApi = async (type, dataUpdate) => {
  ...
  const apiUrl = type === "password" ? apiUpdateCurrentPassword : type === "info" && apiUpdateUserInfo;
  ...
  try {
    ...
    if (data && data.Code === 0) showAlert('success', `${type.toUpperCase()} Updated Successfully`);
